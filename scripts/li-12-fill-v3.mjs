...
const B = [
  ...
  { h: 'The result' },
  {
    p: 'The full suite runs 18 test cases across smoke and regression — and every single one passes. A clean, green Allure report with a 100% pass rate on Chromium, with screenshot, video, and trace captured automatically on any failure.',
  },
  { img: A('allure-overview-allpass.png') },
  { img: A('allure-graph-allpass.png') },
...
await page.screenshot({ path: path.join(out, 'v3-top.png') });
fs.writeFileSync(path.join(out, 'v3-edit-url.txt'), editUrl);
