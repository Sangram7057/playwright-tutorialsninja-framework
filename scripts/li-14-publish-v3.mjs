...
const EDIT_URL = fs.readFileSync(path.join(out, 'v3-edit-url.txt'), 'utf8').trim();

const COMMENTARY =
  "A complete, copy-paste guide: how I built a Playwright + TypeScript framework with Claude Code + MCP — every install step, the MCP setup, the locator strategy, and a green Allure report with all 18 tests passing. If you're doing QA automation, this is the full walkthrough. 👇";
...
await page.screenshot({ path: path.join(out, 'v3-before-publish.png') });
...
await page.screenshot({ path: path.join(out, 'v3-published.png') });
